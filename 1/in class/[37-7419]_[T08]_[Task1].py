...
def crack(cipher):
    ...
    for keyLen in range(1, 11):
        subArray = [""]*keyLen
        for charPos in range(len(cipher)):
            subArray[charPos%keyLen] += cipher[charPos]
        # print(subArray)
        # print('\n')
        mArray.append(subArray)
    res = rafik.array([0])
    for m in mArray:
        res = rafik.append(res,generateIC(m))
    print(rafik.argmax(res))
